...
After writing, reply with: the output path, the resolved transport mode, the resolved **Destination Status** (`confirmed` or `open`, with the place or candidate list), and — if you had to make any assumption significant enough that it should be confirmed with the traveler — a one-line flag naming it. Otherwise just confirm all checklist fields resolved.
